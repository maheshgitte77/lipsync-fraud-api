"""
Lip–audio correlation using MediaPipe face landmarks vs per-frame RMS energy.
Supports legacy `mediapipe.solutions.face_mesh` and newer Tasks API (`FaceLandmarker`).
"""
...
# Face mesh / FaceLandmarker shared topology — lip centers (478-pt model)
_UPPER_LIP = 13
_LOWER_LIP = 14
...
def _resolve_face_landmarker_model() -> Path:
    """Path to .task file; download once to cache if missing."""
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "lipsync-fraud-api/1.0"})
...
def _lip_gap_from_landmark_list(lm_list) -> float | None:
    if not lm_list or len(lm_list) <= max(_UPPER_LIP, _LOWER_LIP):
        return None
    upper = lm_list[_UPPER_LIP].y
    lower = lm_list[_LOWER_LIP].y
    return abs(float(lower - upper))


def _collect_lip_openness_solutions(
    ...
    with face_mesh:
        while True:
            ...
            if result.multi_face_landmarks:
                ...
                gap = _lip_gap_from_landmark_list(lm)
...
def _collect_lip_openness_tasks(
    cap: cv2.VideoCapture,
    *,
    fps: float,
    max_num_faces: int,
...
def analyze_mediapipe_correlation(
    video_path: str | Path,
    *,
    ...
    """
    Returns dict with correlation, passed, frames_processed, optional error.
    """
    ...
    try:
        ...
        if not os.path.isfile(audio_path) or os.path.getsize(audio_path) < 64:
            return {"error": "ffmpeg failed or empty audio", "passed": False}
    ...
    try:
        if hasattr(mp, "solutions") and hasattr(mp.solutions, "face_mesh"):
            lip_openness, backend = _collect_lip_openness_solutions(
                cap,
            ...
            lip_openness, backend = _collect_lip_openness_tasks(
                cap,
                fps=fps,
                max_num_faces=max_num_faces,
                min_detection_confidence=min_detection_confidence,
